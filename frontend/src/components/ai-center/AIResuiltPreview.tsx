    ...
    return (
        <>
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="p-3 md:p-6 min-h-[80vh] max-w-5xl overflow-x-scroll ">
                        ...
                        <div className={`space-y-4 ${totalErrors > 0 && activeFilter !== "invalid" ? "max-h-[300px]" : "max-h-[350px]"} md:max-h-[500px]  overflow-auto`}>
                            {filterQuizData &&
                                filterQuizData.questions.map((question, index) => (
                                    <Card key={question.id} className={`border-l-4 ${Number(question.correct) == -1 ? "border-l-red-700 dark:border-l-red-400 bg-red-500/10" : "border-l-green-700 dark:border-l-green-400 bg-green-500/10"}  hover:shadow-lg transition-shadow duration-200`}>
