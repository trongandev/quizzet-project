...
import { Bot, FileText, Upload, Save, Home, Settings, HelpCircle, FileInput } from "lucide-react";
import { Sidebar, SidebarContent, SidebarGroup, SidebarGroupContent, SidebarGroupLabel, SidebarMenu, SidebarMenuButton, SidebarMenuItem, SidebarHeader, SidebarFooter } from "@/components/ui/sidebar";
import { useRouter } from "next/navigation";

export function SideBarDeCuong() {
    const menuItems = [
        {
            title: "Trang chủ",
            icon: Home,
        ...
        {
            title: "Gửi file đề cương",
            icon: Upload,
            id: "upload-file",
            isHot: true,
            description: "Hỗ trợ docx, xlsx, pdf...",
        },
        {
            ...
            <Sidebar variant="floating" className="mt-20 w-64 h-[calc(100vh-200px)]">
                <SidebarHeader className="p-4">
                    <div className="flex items-center space-x-2">
                        ...
                        <span className="font-bold text-lg">SOMaker</span>
                    </div>
                </SidebarHeader>
                <SidebarContent>
                    <SidebarGroup>
                        <SidebarGroupLabel>Tạo Đề cương</SidebarGroupLabel>
                        <SidebarGroupContent>
                            <SidebarMenu>
                                {menuItems.map((item) => (
                                    <SidebarMenuItem key={item.id}>
                                        <SidebarMenuButton onClick={() => router.push(`/decuong/taodecuong/${item.id}`)} className="w-full justify-start h-14 px-3">
