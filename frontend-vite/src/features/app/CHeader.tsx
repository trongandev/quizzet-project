...
import { BookOpen, Brain, Cpu, FolderOpenDot, History, Home, LogOut, Mailbox, Menu, Moon, Speech, Sun, User } from 'lucide-react'
...
export default function CHeader() {
    ...
    return (
        <>
            ...
            <div className="w-full flex items-center justify-center sticky top-0 z-50 bg-linear-to-br from-primary/20 to-purple-800/20 backdrop-blur-sm py-1 px-3">
                <div className="flex items-center justify-between  mx-auto w-full md:max-w-7xl">
                    <div className="flex items-center gap-2">
                        ...
                        <Link to="/" className="qwigley-font text-4xl translate-y-1 text-primary font-medium">
                            Quizzet
                        </Link>
                    </div>
                    ...
                    <ul className="hidden md:flex items-center gap-5">
                        {linkData.map((link) => (
                            <li key={link.name}>
                                <Link to={link.href} className={`flex gap-1  items-center h-full hover:text-primary transition-colors  ${pathname === link.href ? 'text-primary font-medium' : ''}`}>
                                    {link.icon} {link.name}
                                </Link>
                            </li>
                        ))}
                        ...
                        {!user ? (
                            <div className=" ">
                                <Link to="/login" className="relative">
                                    ...
                                    <Button className="z-1 relative group overflow-hidden bg-linear-to-br from-primary/80 to-purple-700/80 font-bold group-hover:scale-105 transition-all duration-500 px-4 py-2 rounded-lg text-white">
                                        Đăng nhập
