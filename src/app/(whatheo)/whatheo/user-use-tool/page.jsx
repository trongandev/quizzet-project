...
export default function UserUseTool() {
    ...
    const removeDoc = async (id) => {
        const req = await POST_API(`/tool/user`, { id: id }, "DELETE", token);
    ...
    const handleUpdateUser = async () => {
        setConfirmLoading(true);
        const req = await POST_API(`/tool/user/${dataDetail._id}`, dataDetail, "PATCH", token);
        const res = await req.json();
        if (req.ok) {
            setOpenDetail(false);
    ...
    const handlePost = async () => {
        ...
        const res = await POST_API("/tool/user", data, "POST", token);
    ...
    const handleChangeStatus = async (id, status) => {
        ...
        const req = await POST_API(`/tool/user/${id}`, { status: !status }, "PATCH", token);
        const res = await req.json();
        if (req.ok) {
    ...
    const onChange = async (value, id) => {
        setLoadingSwitch(true);
        const req = await POST_API(`/tool/user/${id}`, { count_login: value }, "PATCH", token);
        const res = await req.json();
        if (req.ok) {
            const newTool = tool.map((item) => {
                if (item._id === id) {
    ...
    const onChangeFail = async (value, id) => {
        setLoadingSwitch(true);
        const req = await POST_API(`/tool/user/${id}`, { failed_login_attempts: value }, "PATCH", token);
        const res = await req.json();
        if (req.ok) {
            const newTool = tool.map((item) => {
                if (item._id === id) {
