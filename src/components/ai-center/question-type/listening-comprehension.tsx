        ...
        <div className="space-y-6">
            ...
            <Card className="bg-slate-800 border-slate-700">
                <CardContent className="p-6">
                    ...
                    <div className=" mb-3">{edit ? <Textarea value={editData.question_text} onChange={(e) => setEditData({ ...editData, question_text: e.target.value })} className="bg-slate-700 border-slate-600 text-white h-16 " /> : <h3 className="text-xl font-semibold text-white">{question.question_text}</h3>}</div>
                    {edit ? (
                        <>
                            <h4 className="font-medium text-slate-300 mb-1">Nhập nội dung để phát âm thanh:</h4>
                            <Textarea value={editData.audio_text} onChange={(e) => setEditData({ ...editData, audio_text: e.target.value })} className="bg-slate-700 border-slate-600 text-white h-[100px] " />
                        </>
                    ) : (
                    ...
                    <div className="space-y-2 mt-3">
                        ...
                        {edit ? <Textarea value={question.correct_answer_text} className="bg-slate-700 border-slate-600 text-white h-[100px] text-lg leading-relaxed" placeholder="Nhập câu trả lời dựa trên nội dung âm thanh..." /> : <p>{question.correct_answer_text}</p>}
                    </div>
                    {edit && (
