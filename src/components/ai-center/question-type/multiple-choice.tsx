    ...
    return (
        <Card className="bg-slate-800 border-slate-700">
            <CardContent className="p-6">
                ...
                <h3 className="text-xl font-semibold text-white mb-6">{edit ? <Textarea className="h-16" value={editData.question_text} onChange={(e) => setEditData({ ...editData, question_text: e.target.value })} /> : <>{question.question_text}</>}</h3>
                ...
                <div className="space-y-3">
                    ...
                    {edit &&
                        editData.options.map((option: any) => (
                            <Button key={option.id} variant="outline" className={`w-full justify-start h-auto p-4 text-left transition-all ${editData.correct_answer_id === option.id ? "border-blue-500 bg-blue-500/10 text-blue-400" : "border-slate-600 text-slate-300 hover:border-slate-500 hover:bg-slate-700"}`}>
                                <div className="flex items-center gap-3 w-full">
                                    <div
                                        className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${editData.correct_answer_id === option.id ? "border-blue-500 bg-blue-500" : "border-slate-500"}`}
                                        onClick={() => {
