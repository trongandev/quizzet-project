        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 h-[500px] overflow-y-auto my-5">
                {levels.map((level, index) => {
                    ...
                    return (
                        <Card key={index} className={`dark:border-slate-700 border-2 ${isCurrentLevel ? "bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 border-indigo-600/50 dark:border-indigo-500/50" : isUnlocked ? "dark:bg-slate-800" : "dark:bg-slate-800/50"}`}>
                            <CardContent className="p-6 text-center">
                                <div className={`w-20 h-20 mx-auto mb-4 rounded-full flex items-center justify-center  ${isUnlocked ? `bg-gradient-to-br from-indigo-400 to-indigo-600` : " bg-slate-400/50 dark:bg-slate-700"} ${isCurrentLevel ? "ring-4 ring-indigo-500/50" : ""}`}>
                                    <Image src={level.levelIcon} alt="" width={40} height={40} />
                                </div>
