...
export default function AddMoreVocaModal({ children, listFlashcard, setListFlashcard, token, filteredFlashcards, setFilteredFlashcards, speakWord }: AddVocabularyModalProps) {
    ...
    const handleSendData = async () => {
        try {
            setLoading(true);
            toast.loading("Đang gửi dữ liệu lên server, vui lòng đợi trong giây lát...", {
                description: "Quá trình này có thể mất vài giây đến vài phút tùy vào số lượng từ vựng.",
                id: "ai-generate",
            });
            const req = await POST_API("/flashcards/create-ai-list", { list_flashcard_id: listFlashcard._id, language: listFlashcard?.language, data: flashcards || "" }, "POST", token);
                        ...
                        <Card className="border-blue-100 bg-blue-50/30 dark:bg-slate-800/50 dark:border-white/10">
                            <CardContent className="space-y-4 mt-6">
                                ...
                                <div className="flex justify-between items-center bg-gray-500  dark:bg-slate-600/50 p-3 rounded-lg">
                                    <p className="text-xs text-gray-300 dark:text-white/50">Các từ vựng cách nhau bằng dấu phẩn</p>
                                    <span className="text-xs text-gray-400">
                                        {vocabulary.split(",").length}/24 từ {" | Mất khoảng:" + Math.floor(vocabulary.split(" ").length * 1.4 + vocabulary.split(" ").length) + "s"}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center bg-gray-500 dark:bg-slate-600/50 p-3 rounded-lg">
                                    <p className="text-xs text-gray-300 dark:text-white/50">Các câu cách nhau bằng dấu chấm phẩy</p>
                                            ...
                                            <CardHeader>
                                                <CardTitle className="flex items-center flex-wrap gap-2">
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => speakWord(fc.title, index)}>
